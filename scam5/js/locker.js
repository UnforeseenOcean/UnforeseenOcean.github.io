...
CPBContentLocker.prototype = {
    ...
    checkLead: function () {
        ...
        script.type = 'text/javascript';
        script.src = this.protocol + this.domain + "/public/external/check.php?time=" + new Date().getTime() + "&it=" + this.settings.it;
        this.head.appendChild(script);
    },
    ...
    setURLS:function(){
        this.urls.iframe=this.protocol+(this.settings.testing==1 ? "staging." : "")+this.domain+"/public/offers/customTemplates.php?it=" + encodeURI(this.settings.it) + "&w=" + encodeURI(window.screen.availWidth) + "&h=" + encodeURI(window.screen.availHeight) + "&key=" + encodeURI(this.settings.key) + "&m=" + encodeURI((this.isMobile) ? 1 : 0) + this.paramsToQuery(this.extraParams) + "&r=" + encodeURI(this.referrer);
        this.urls.feed=this.protocol+(this.settings.testing==1 ? "staging." : "")+this.domain+"/public/offers/feed_it.php?it=" + encodeURI(this.settings.it) + "&w=" + encodeURI(window.screen.availWidth) + "&h=" + encodeURI(window.screen.availHeight) + "&key=" + encodeURI(this.settings.key) + "&m=" + encodeURI((this.isMobile) ? 1 : 0) + this.paramsToQuery(this.extraParams) + "&callback=?";
        this.urls.iframeOnly=this.protocol+(this.settings.testing==1 ? "staging." : "")+this.domain+"/public/offers/iframe.php?it=" + encodeURI(this.settings.it) + "&w=" + encodeURI(window.screen.availWidth) + "&h=" + encodeURI(window.screen.availHeight) + "&key=" + encodeURI(this.settings.key) + "&m=" + encodeURI((this.isMobile) ? 1 : 0) + this.paramsToQuery(this.extraParams) + "&r=" + encodeURI(this.referrer);
        this.urls.js=this.protocol+(this.settings.testing==1 ? "staging." : "")+this.domain+"/public/external/v2/"+((this.settings.preview === true) ? "preview" : 'html') + "." + this.settings.it + "." + this.settings.key + "." + ((this.isMobile) ? 1 : 0) + ".js" +((this.settings.preview === true) ? ("?t=" + encodeURI(new Date().getTime())) : '');
        this.urls.css=this.protocol+(this.settings.testing==1 ? "staging." : "")+this.domain+"/public/external/css_front.css";
        this.urls.specific_css=this.protocol+(this.settings.testing==1 ? "staging." : "")+this.domain+"/public/clockers/";
...
function CPABuildCheckForLead() {
    CPABUILDContentLocker.checkLead();
}
