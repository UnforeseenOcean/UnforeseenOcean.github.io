...
function svgdom(query, attrs, childs) {
  ...
  var campaignKey = "iR0GQ",
      animationMilliseconds = ie89() ? 0 : 300,
      imageButton = false,
      widgetName = "Gleam",
      buttonPosition = "bottom-right",
      widgetTheme = "floating",
      overlayBackgroundColor = "#cf4647",
      overlayTextColor = "white",
      allowRemove = false,
      popupUrl = "https://gleam.io/iR0GQ/popup",
      overlayText = "Demo: Test Drive a Competition",
      standAloneUrl = "https://gleam.io/iR0GQ/gleam-demo-competition",
  ...
  function addOverlay() {
    ...
    function repositionWindow(height) {
      ...
      var trueHeight = Math.min(extWindowHeight() - 20, override ? override : height);
      ...
      if(popup.getAttribute('state') === 'present') {
        popup.style.top = (((extWindowHeight() - trueHeight) / 2) - 5) + 'px';
      }
    }
    ...
    popup.style.top = '-' + extWindowHeight() + 'px';
    ...
    widget()['openPopupCampaign' + campaignKey] = button.onclick = function () {
      if(smallScreen()) {
        window.open(withLocation(withEnterId(standAloneUrl)), 'gleam', 'menubar=no,toolbar=no,resizable=no,scrollbars=yes,height='+ extWindowHeight() +',width=540');
        return false;
      } else {
    ...
    function showPopup(){
      ...
      repositionWindow(extWindowHeight() - 20);
    }
...
function push(vars) {
    ...
    if(getCookie('GleamId')) {
        var src = 'https://gleam.io/track.js?i='+encodeURIComponent(getCookie('GleamId'))+"&" +
            "k="+campaignKeys().join(',')+"&"+
